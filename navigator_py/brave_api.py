...
__all__ = ["BraveRequest", "BraveResult"]
...
class BraveResult:
    ...
    def get_links(self, n: int = 3) -> list:
        """Get the top n links from the Brave API response."""
        return [{result.get("title"): result.get("url")} for result in self.top(n)]
